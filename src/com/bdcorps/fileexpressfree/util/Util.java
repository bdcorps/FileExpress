...
public final class Util {
	...
	public static Drawable getIcon(Context mContext, File file) {
		...
		} else // file
		{
			...
			if (fileName.endsWith(".apk")) {
				...
				return mContext.getResources().getDrawable(
						R.drawable.filetype_pdf);
			} else if (fileName.endsWith(".html")) {
				return mContext.getResources().getDrawable(
						R.drawable.filetype_html);
			} else if (fileName.endsWith(".txt")) {
				return mContext.getResources().getDrawable(
						R.drawable.filetype_txt);
			}
					...
					try {
						...
						Log.e(TAG,
								"Error while deleting directory after paste - "
										+ fileBeingPasted.getAbsolutePath(), e);
						return false;
					}
				}
			}
	...
	private static boolean doPaste(int mode, File srcFile, File destinationDir,
			AbortionFlag flag) {
	...
	public static boolean canShowQuickActions(FileListEntry currentFile,
			FileListActivity mContext) {

		if (!mContext.getPreferenceHelper().useQuickActions()
				|| mContext.isInPickMode()) {
			return false;
		}
	...
	public static CharSequence[] getFileProperties(FileListEntry file,
			FileListActivity context) {
		...
		if (Util.isSdCard(file.getPath())) {
			...
			return new CharSequence[] {
					context.getString(R.string.total_capacity,
							Util.getSizeStr(totalSize)),
					context.getString(R.string.free_space,
							Util.getSizeStr(sdAvailSize)) };
		} else if (file.getPath().isFile())
			return new CharSequence[] {
					context.getString(R.string.filepath_is, file.getPath()
							.getAbsolutePath()),
					context.getString(
							R.string.mtime_is,
							DateFormat.getDateFormat(context).format(
									file.getLastModified())),
					context.getString(R.string.size_is,
							FileUtils.byteCountToDisplaySize(file.getSize())) };
	...
	public static void gotoPath(final String currentPath,
	...
	public static void gotoPath(final String currentPath,
			final FileListActivity mContext, final CancellationCallback callback) {
						...
						new DialogInterface.OnClickListener() {
							public void onClick(DialogInterface dialog,
								...
								try {
									...
									Log.e(TAG, "Error navigating to path"
											+ toPath, e);
									new Builder(mContext)
											.setTitle(
													mContext.getString(R.string.error))
											.setMessage(
													mContext.getString(R.string.path_not_exist))
											.show();
								}
							}
						})
