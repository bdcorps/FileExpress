...
import com.bdcorps.fileexpressfree.activity.BookmarkListActivity;
import com.bdcorps.fileexpressfree.model.FileListEntry;
import com.bdcorps.fileexpressfree.util.FileListSorter;
...
public class BookmarkLoader extends
		AsyncTask<File, Integer, List<FileListEntry>> {
		...
		caller.runOnUiThread(new Runnable() {
			...
			public void run() {
				...
				if (childFiles.size() > 0) {
					caller.getActionBar().setSubtitle(
							caller.getString(R.string.bookmarks_count,
									childFiles.size()));
				} else {
					caller.getActionBar().setSubtitle(
							caller.getString(R.string.bookmarks_count_0));
				}
			}
		});
	...
	protected List<FileListEntry> doInBackground(File... params) {
		...
		List<FileListEntry> childFiles = new ArrayList<FileListEntry>(caller
				.getBookmarker().getBookmarks());
