...
public class Unzipper extends AsyncTask<File, String, List<String>> {
	...
	protected void onPostExecute(final List<String> result) {
		mContext.runOnUiThread(new Runnable() {
			...
			public void run() {
				...
				boolean unzippedAtleastOne = (result != null)
						&& (result.size() > 0);
				...
				if (flag.isAborted()) {
					mContext.refresh();
					String message = unzippedAtleastOne ? mContext.getString(
							R.string.unzip_abort_partial,
							destination.getAbsolutePath()) : mContext
							.getString(R.string.unzip_aborted);
					...
					new Builder(mContext)
							...
							.setMessage(
									mContext.getString(R.string.unzip_success,
											result.size()))
							.setPositiveButton(android.R.string.ok,
									new OnClickListener() {
	...
	protected void onPreExecute() {
		...
		mContext.runOnUiThread(new Runnable() {
			...
			public void run() {
				...
				zipProgress.setMessage(mContext
						.getString(R.string.unzip_progress));
				zipProgress.setButton(
						mContext.getString(R.string.run_in_background),
						new OnClickListener() {
				...
				zipProgress.setButton2(mContext.getString(R.string.cancel),
						new OnClickListener() {
							...
							public void onClick(DialogInterface dialog,
								...
								zipProgress.getButton(which).setText(
										R.string.cancelling);
		...
		if (files != null && files.length >= 1) {
			...
			for (File zipFile : files) {
				try {
					...
					Log.e(TAG, "Failed to unzip " + zipFile.getAbsolutePath()
							+ ". File exists - " + zipFile.exists(), e);
					mContext.runOnUiThread(new Runnable() {
						...
						public void run() {
							...
							new Builder(mContext)
									...
									.setMessage(
											mContext.getString(R.string.unzip_failed_generic))
									.setPositiveButton(android.R.string.ok,
											new OnClickListener() {
		...
		if (values != null && values.length > 0) {
			...
			mContext.runOnUiThread(new Runnable() {
				...
				public void run() {
					...
					if (zipProgress != null && zipProgress.isShowing()) {
						zipProgress.setMessage(mContext.getString(
								R.string.unzip_progress, curr));
					}
				}
			});
		}
	...
	private void extractFolder(File zipFile, List<String> extracted)
		...
		String newPath = destination.getAbsolutePath()
				+ "/"
				+ zipFile.getName()
						.substring(0, zipFile.getName().length() - 4);
